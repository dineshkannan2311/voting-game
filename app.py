...
def init_database():
    """Initialize with EMPTY database - ready for your data!"""
...
def get_current_state():
    ...
    try:
        ...
        # If showing results, include leaderboard (ONE WINNER PER TEAM, NO VOTE COUNTS)
...
def calculate_leaderboard(db):
    ...
    # Get ONE winner per team (highest votes in each team)
    winners = {}
    for name, vote_count in counts.items():
        if name in info:
            tid = info[name]['team_id']
            # Only keep if this person has MORE votes than current team winner
            if tid not in winners or vote_count > winners[tid]['votes']:
                winners[tid] = {
                    'team_id': tid,
                    'team': info[name]['team_name'],
                    'name': name,
                    'votes': vote_count  # Internal use only, won't show to users
                }
...
if __name__ == '__main__':
    ...
    print("🎮 OFFICE LEGENDS - ULTIMATE EDITION")
