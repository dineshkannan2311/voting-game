...
def fuzzy_match(name1, name2, threshold=0.85):
    """
    Fuzzy match two names with 85% threshold
    ...
    # "Dinesh K" should match "Dinesh Kannan"
    ...
    for short_part in shorter_parts:
        for long_part in longer_parts:
            # Check if abbreviated (first letter matches) or similar
            if short_part == long_part or short_part[0] == long_part[0]:
                matches += 1
                break
...
def timer_worker():
    ...
    while timer_active:
        ...
        print(f"⏱️ Timer: {remaining}s remaining")
        ...
        if remaining <= 0:
            ...
            print(f"🏆 Results sent! Winners: {len(leaderboard)}")
            break
...
def login():
    ...
    for member in db['members']:
        if fuzzy_match(name, member['name']):
            # Calculate exact ratio for best match
    ...
    if matched_member:
        ...
        return jsonify({
            'success': False,
            'message': f'No match found for "{name}". Please check spelling or contact admin.'
        }), 404
...
def submit_vote():
    ...
    if existing_vote:
        return jsonify({'success': False, 'message': 'You have already voted for this question'}), 400
...
def broadcast_question():
    ...
    # Clear previous votes for this question
    ...
    print(f"📢 Broadcast sent: {question['text']}")
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("🎮 OFFICE VOTING GAME - BACKEND SERVER")
    print("=" * 60)
